...
type TicketQueue = 'all' | 'unassigned' | 'mine' | 'overdue'
...
type Conversation = { _id: string; subject?: string; status: Status; mode: 'ai' | 'human'; priority?: Priority; category?: Category; slaDeadline?: string; lastMessageAt: string; customer?: { name: string; email: string }; assignedAgent?: { _id: string; name: string } }
type AttachmentMetadata = { fileName?: string; mimeType?: string; size?: number }
type Citation = { sourceType: 'article' | 'document'; title: string }
type Message = { _id: string; content: string; senderType: 'customer' | 'agent' | 'ai' | 'system'; messageType?: 'message' | 'file' | 'system' | 'internal_note'; metadata?: AttachmentMetadata & { citations?: Citation[] }; createdAt: string }
type Article = { _id: string; title: string; content: string; tags: string[]; isPublished?: boolean; updatedAt: string }
type KnowledgeDocument = { _id: string; title: string; tags: string[]; isPublished: boolean; archivedAt?: string | null; chunkCount: number }
type KnowledgeAnalytics = { publishedArticles: number; publishedDocuments: number; chunkCount: number; topCitedSourceTitles: Array<{ title: string; sourceType: string; count: number }>; gapSuggestions: Array<{ terms: string[]; count: number }> }
type OperationalAnalytics = { openPending: number; overdueSla: number; firstResponseProxy: number; averageFirstResponseMinutes: number; byCategory: Array<{ category: string; count: number }>; byPriority: Array<{ priority: string; count: number }> }
...
function App() {
  ...
  const [ticketQueue, setTicketQueue] = useState<TicketQueue>('all')
  ...
  const shownConversations = useMemo(() => conversations.filter((item) => {
    ...
    const inQueue = ticketQueue === 'all' || (ticketQueue === 'unassigned' && !item.assignedAgent) || (ticketQueue === 'mine' && item.assignedAgent?._id === user?.id) || (ticketQueue === 'overdue' && ['open', 'pending'].includes(item.status) && Boolean(item.slaDeadline && new Date(item.slaDeadline).getTime() < Date.now()))
    return inQueue && (!query || searchable.includes(query)) && (statusFilter === 'all' || item.status === statusFilter) && (priorityFilter === 'all' || (item.priority ?? 'normal') === priorityFilter)
  }), [conversations, priorityFilter, statusFilter, ticketQuery, ticketQueue, user?.id])

  const logout = useCallback(() => {
    localStorage.removeItem(TOKEN_KEY)
    setToken(''); setUser(null); setActive(null); setMessages([]); setConversations([]); setAccounts([]); setArticles([]); setDocuments([]); setAnalytics(null); setOperationalAnalytics(null); setNotice('')
  }, [])
  ...
  const loadKnowledgeAdmin = useCallback(async () => {
    try {
      const [documentData, analyticsData, operationsData] = await Promise.all([
        request<Api<{ documents: KnowledgeDocument[] }>>('/api/knowledge-base/documents', token),
        request<Api<{ analytics: KnowledgeAnalytics }>>('/api/knowledge-base/analytics', token),
        request<Api<{ analytics: OperationalAnalytics }>>('/api/operations/tickets', token),
      ])
      setDocuments(documentData.documents); setAnalytics(analyticsData.analytics); setOperationalAnalytics(operationsData.analytics)
    } catch (error) { setNotice(errorText(error)) }
  }, [token])
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        const response = await fetch(`${API}/api/realtime/events`, { headers: { Authorization: `Bearer ${token}` }, signal: controller.signal, cache: 'no-store' })
        ...
        while (!controller.signal.aborted) {
          ...
          for (const event of events) {
            if (!/event: (notification|conversation|ai_status)/.test(event)) continue
            if (/event: ai_status/.test(event)) { setAiWorking(true); continue }
            if (/event: conversation/.test(event)) setAiWorking(false)
            void loadNotifications(); void loadConversations()
            if (active) void loadMessages(active._id)
          }
        }
  ...
  }, [active, loadConversations, loadMessages, loadNotifications, token])
  ...
  useEffect(() => { if ((view === 'accounts' || view === 'inbox') && isAdmin) void loadAccounts() }, [view, isAdmin, loadAccounts])
  ...
  useEffect(() => { if (view === 'knowledge' && isAdmin) void loadKnowledgeAdmin() }, [view, isAdmin, loadKnowledgeAdmin])
  ...
  async function reassignConversation(assignedAgentId: string) {
    if (!active || !assignedAgentId) return; setBusy(true)
    try {
      const data = await request<Api<{ conversation: Conversation }>>(`/api/conversations/${active._id}/handoff`, token, { method: 'POST', body: JSON.stringify({ assignedAgentId }) })
      setActive(data.conversation); await Promise.all([loadMessages(active._id), loadConversations()]); setNotice('Đã gán ticket cho Agent.')
    } catch (error) { setNotice(errorText(error)) } finally { setBusy(false) }
  }
  async function generateAgentReplyDraft() {
    if (!active) return
    setBusy(true)
    try { const data = await request<Api<{ draft: string }>>(`/api/conversations/${active._id}/ai-draft`, token, { method: 'POST' }); setDraft(data.draft); setNotice('AI đã tạo bản nháp. Hãy rà soát trước khi gửi.') } catch (error) { setNotice(errorText(error)) } finally { setBusy(false) }
  }
  ...
  async function generateFaq(topic: string) {
    setBusy(true)
    try { const data = await request<Api<{ article: Article }>>('/api/knowledge-base/generate-faq', token, { method: 'POST', body: JSON.stringify({ topic }) }); setEditingArticle({ ...data.article, _id: '', updatedAt: '' }); setNotice('Đã tạo bản nháp FAQ. Hãy rà soát trước khi xuất bản.') } catch (error) { setNotice(errorText(error)) } finally { setBusy(false) }
  }
  async function createDocument(document: { title: string; content: string; tags: string[]; isPublished: boolean }) {
    setBusy(true)
    try { await request('/api/knowledge-base/documents', token, { method: 'POST', body: JSON.stringify(document) }); await loadKnowledgeAdmin(); setNotice('Đã thêm tài liệu nội bộ.') } catch (error) { setNotice(errorText(error)) } finally { setBusy(false) }
  }
  async function archiveDocument(document: KnowledgeDocument) {
    if (!window.confirm(`Lưu trữ tài liệu “${document.title}”? Tài liệu sẽ không còn được AI truy xuất.`)) return
    setBusy(true)
    try { await request(`/api/knowledge-base/documents/${document._id}`, token, { method: 'DELETE' }); await loadKnowledgeAdmin(); setNotice('Đã lưu trữ tài liệu.') } catch (error) { setNotice(errorText(error)) } finally { setBusy(false) }
  }
  ...
  return <main className="app-frame">
    ...
    {view === 'accounts' && isAdmin ? <AccountManager {...{ accounts, accountQuery, setAccountQuery, accountRole, setAccountRole, loadAccounts, updateAccount, user, busy }} />
      : view === 'knowledge' && isAdmin ? <KnowledgeBase {...{ articles, articleQuery, setArticleQuery, loadArticles, editingArticle, setEditingArticle, saveArticle, deleteArticle, generateFaq, documents, createDocument, archiveDocument, analytics, operationalAnalytics, busy }} />
        : view === 'widget' || !isStaff ? <CustomerPortal {...{ user, customerTab, setCustomerTab, conversations, conversationsLoading, active, selectConversation: setActive, messages, subject: newSubject, setSubject: setNewSubject, draft, setDraft, createConversation, sendMessage, uploadAttachment, downloadAttachment, handoff, aiWorking, articles, articleQuery, setArticleQuery, loadArticles, articlesLoading, busy }} />
          : <AgentInbox {...{ conversations: shownConversations, allConversations: conversations, active, messages, ticketQuery, setTicketQuery, ticketQueue, setTicketQueue, statusFilter, setStatusFilter, priorityFilter, setPriorityFilter, selectConversation: setActive, draft, setDraft, sendMessage, generateAgentReplyDraft, updateStatus, updateMetadata, addInternalNote, handoff, isAdmin, agents: accounts.filter((account) => account.isActive && (account.role === 'agent' || account.role === 'admin')), reassignConversation, busy }} />}
...
function AgentInbox(props: { conversations: Conversation[]; allConversations: Conversation[]; active: Conversation | null; messages: Message[]; ticketQuery: string; setTicketQuery: (value: string) => void; ticketQueue: TicketQueue; setTicketQueue: (value: TicketQueue) => void; statusFilter: 'all' | Status; setStatusFilter: (value: 'all' | Status) => void; priorityFilter: 'all' | Priority; setPriorityFilter: (value: 'all' | Priority) => void; selectConversation: (conversation: Conversation) => void; draft: string; setDraft: (value: string) => void; sendMessage: (event: FormEvent) => void; generateAgentReplyDraft: () => void; updateStatus: (status: Status) => void; updateMetadata: (changes: Partial<Pick<Conversation, 'priority' | 'category'>>) => void; addInternalNote: (content: string) => void; handoff: () => void; isAdmin: boolean; agents: Account[]; reassignConversation: (assignedAgentId: string) => void; busy: boolean }) {
  const p = props
  return <section className="workspace inbox-workspace"><section className="inbox-grid">
    <aside className="ticket-list"><header className="panel-header"><div><p className="eyebrow">TICKET QUEUE</p><h2>Hộp thư đến</h2></div><span className="count-badge">{p.conversations.length}</span></header><div className="queue-tabs"><button className={p.ticketQueue === 'all' ? 'selected' : ''} onClick={() => p.setTicketQueue('all')}>Tất cả</button><button className={p.ticketQueue === 'unassigned' ? 'selected' : ''} onClick={() => p.setTicketQueue('unassigned')}>Chưa nhận</button><button className={p.ticketQueue === 'mine' ? 'selected' : ''} onClick={() => p.setTicketQueue('mine')}>Của tôi</button><button className={p.ticketQueue === 'overdue' ? 'selected' : ''} onClick={() => p.setTicketQueue('overdue')}>Quá SLA</button></div><input className="ticket-search" value={p.ticketQuery} onChange={(e) => p.setTicketQuery(e.target.value)} placeholder="Tìm ticket hoặc khách hàng" /><div className="ticket-filters"><select value={p.statusFilter} onChange={(e) => p.setStatusFilter(e.target.value as 'all' | Status)}><option value="all">Mọi trạng thái</option>{statuses.map((item) => <option key={item} value={item}>{labelStatus(item)}</option>)}</select><select value={p.priorityFilter} onChange={(e) => p.setPriorityFilter(e.target.value as 'all' | Priority)}><option value="all">Mọi ưu tiên</option>{priorities.map((item) => <option key={item} value={item}>{labelPriority(item)}</option>)}</select></div><div className="tickets">{p.conversations.map((item) => <button className={item._id === p.active?._id ? 'ticket selected-ticket' : 'ticket'} onClick={() => p.selectConversation(item)} key={item._id}><div className="ticket-meta"><small>#{item._id.slice(-6).toUpperCase()}</small><span className={`priority-dot ${item.priority ?? 'normal'}`}>{labelPriority(item.priority ?? 'normal')}</span></div><strong>{item.subject || 'Yêu cầu hỗ trợ'}</strong><p>{item.customer?.name ?? 'Khách hàng'} · {relativeTime(item.lastMessageAt)}</p><footer><span className={`tiny-status ${item.status}`}>{labelStatus(item.status)}</span><span>{item.mode === 'human' ? 'Agent' : 'AI'}</span></footer></button>)}{!p.conversations.length && <p className="empty">Không có ticket phù hợp.</p>}</div></aside>
    <section className="conversation-panel">{p.active ? <><header className="conversation-header"><div><p className="eyebrow">#{p.active._id.slice(-6).toUpperCase()}</p><h2>{p.active.subject || 'Yêu cầu hỗ trợ'}</h2><p>{p.active.customer?.name ?? 'Khách hàng'} · {p.active.customer?.email ?? 'Chưa có email'}</p></div><div className="conversation-actions"><span className={`status-pill ${p.active.status}`}>{labelStatus(p.active.status)}</span>{(p.active.mode === 'ai' || !p.active.assignedAgent) && <button className="action outline" onClick={p.handoff} disabled={p.busy}>Nhận ticket</button>}{p.isAdmin && <label className="agent-reassign"><span>Gán Agent</span><select value={p.active.assignedAgent?._id ?? ''} onChange={(event) => { if (event.target.value) p.reassignConversation(event.target.value) }} disabled={p.busy}><option value="">Chưa phân công</option>{p.agents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label>}</div></header><div className="chat-history agent-history">{p.messages.length === 0 && <WelcomeMessage />}{p.messages.map((message) => <MessageBubble key={message._id} message={message} />)}</div><form className="agent-composer" onSubmit={p.sendMessage}><textarea value={p.draft} onChange={(e) => p.setDraft(e.target.value)} placeholder="Trả lời khách hàng…" rows={3} /><div><span>AI chỉ tạo bản nháp; hãy rà soát trước khi gửi.</span><button type="button" className="action outline" onClick={p.generateAgentReplyDraft} disabled={p.busy}>✨ AI gợi ý</button><button className="action primary" disabled={p.busy || !p.draft.trim()}>Gửi phản hồi</button></div></form></> : <EmptyConversation />}</section>
...
function KnowledgeBase({ articles, articleQuery, setArticleQuery, loadArticles, editingArticle, setEditingArticle, saveArticle, deleteArticle, generateFaq, documents, createDocument, archiveDocument, analytics, operationalAnalytics, busy }: { articles: Article[]; articleQuery: string; setArticleQuery: (value: string) => void; loadArticles: (query?: string) => Promise<void>; editingArticle: Article | null; setEditingArticle: (article: Article | null) => void; saveArticle: (article: Omit<Article, '_id' | 'updatedAt'>, id?: string) => Promise<void>; deleteArticle: (article: Article) => Promise<void>; generateFaq: (topic: string) => Promise<void>; documents: KnowledgeDocument[]; createDocument: (document: { title: string; content: string; tags: string[]; isPublished: boolean }) => Promise<void>; archiveDocument: (document: KnowledgeDocument) => Promise<void>; analytics: KnowledgeAnalytics | null; operationalAnalytics: OperationalAnalytics | null; busy: boolean }) {
  return <section className="workspace knowledge-workspace"><section className="workspace-title"><div><p className="kicker">ADMIN CONTENT OPERATIONS</p><h1>Knowledge base</h1><p>Xây dựng câu trả lời chuẩn để hỗ trợ đội ngũ và AI assistant.</p></div><button className="action primary" onClick={() => setEditingArticle({ _id: '', title: '', content: '', tags: [], isPublished: false, updatedAt: '' })}>+ Bài viết mới</button></section><FaqGenerator onGenerate={generateFaq} busy={busy} /><section className="knowledge-grid"><section className="article-library"><div className="library-toolbar"><input value={articleQuery} onChange={(e) => setArticleQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') void loadArticles(articleQuery) }} placeholder="Tìm trong knowledge base" /><button className="action outline" onClick={() => void loadArticles(articleQuery)} disabled={busy}>Tìm</button></div><p className="library-count">{articles.length} bài viết đã xuất bản</p><div className="article-list">{articles.map((article) => <article className={editingArticle?._id === article._id ? 'article-row selected-article' : 'article-row'} key={article._id}><button onClick={() => setEditingArticle(article)}><strong>{article.title}</strong><p>{article.content.slice(0, 150)}{article.content.length > 150 ? '…' : ''}</p><div>{article.tags.map((tag) => <span key={tag} className="tag">{tag}</span>)}</div><small>Cập nhật {new Date(article.updatedAt).toLocaleDateString('vi-VN')}</small></button><button className="icon-button" aria-label={`Xóa ${article.title}`} onClick={() => void deleteArticle(article)} disabled={busy}>×</button></article>)}{!articles.length && <p className="empty">Chưa tìm thấy bài viết.</p>}</div></section><ArticleEditor key={editingArticle?._id ?? 'new'} article={editingArticle} onCancel={() => setEditingArticle(null)} onSave={saveArticle} busy={busy} /></section><section className="kb-admin-panels"><DocumentLibrary documents={documents} createDocument={createDocument} archiveDocument={archiveDocument} busy={busy} /><KnowledgeAnalyticsPanel analytics={analytics} /><OperationsAnalyticsPanel analytics={operationalAnalytics} /></section></section>
}

function FaqGenerator({ onGenerate, busy }: { onGenerate: (topic: string) => Promise<void>; busy: boolean }) { const [topic, setTopic] = useState(''); return <form className="kb-admin-card" onSubmit={(event) => { event.preventDefault(); if (topic.trim()) void onGenerate(topic.trim()) }}><div><p className="eyebrow">AI FAQ DRAFT</p><strong>Tạo bản nháp, rồi rà soát và xuất bản thủ công</strong></div><input value={topic} onChange={(event) => setTopic(event.target.value)} maxLength={160} required placeholder="Chủ đề, ví dụ: đặt lại mật khẩu" /><button className="action outline" disabled={busy || !topic.trim()}>Tạo bản nháp</button></form> }
function DocumentLibrary({ documents, createDocument, archiveDocument, busy }: { documents: KnowledgeDocument[]; createDocument: (document: { title: string; content: string; tags: string[]; isPublished: boolean }) => Promise<void>; archiveDocument: (document: KnowledgeDocument) => Promise<void>; busy: boolean }) { const [title, setTitle] = useState(''); const [content, setContent] = useState(''); const [tags, setTags] = useState(''); const [isPublished, setPublished] = useState(true); return <section className="kb-admin-card"><header><div><p className="eyebrow">DOCUMENT LIBRARY</p><h2>Tài liệu nội bộ</h2></div></header><form className="document-form" onSubmit={(event) => { event.preventDefault(); void createDocument({ title, content, tags: tags.split(',').map((tag) => tag.trim()).filter(Boolean), isPublished }).then(() => { setTitle(''); setContent(''); setTags('') }) }}><input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={200} required placeholder="Tiêu đề tài liệu" /><textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={50000} required placeholder="Chỉ dán nội dung văn bản nội bộ — không tải tệp" /><input value={tags} onChange={(event) => setTags(event.target.value)} placeholder="Thẻ, ngăn cách bằng dấu phẩy" /><label className="publish-toggle"><input type="checkbox" checked={isPublished} onChange={(event) => setPublished(event.target.checked)} /> Cho phép AI dùng tài liệu này</label><button className="action primary" disabled={busy}>Thêm tài liệu văn bản</button></form><div className="document-list">{documents.map((document) => <article key={document._id}><div><strong>{document.title}</strong><small>{document.chunkCount} chunks · {document.archivedAt ? 'Đã lưu trữ' : document.isPublished ? 'Đang xuất bản' : 'Bản nháp'}</small><div>{document.tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div></div>{!document.archivedAt && <button className="action outline" onClick={() => void archiveDocument(document)} disabled={busy}>Lưu trữ</button>}</article>)}{!documents.length && <p className="empty">Chưa có tài liệu nội bộ.</p>}</div></section> }
function KnowledgeAnalyticsPanel({ analytics }: { analytics: KnowledgeAnalytics | null }) { if (!analytics) return <section className="kb-admin-card"><p className="empty">Đang tải analytics…</p></section>; return <section className="kb-admin-card"><p className="eyebrow">KB ANALYTICS</p><div className="kb-metrics"><Metric label="Bài đã xuất bản" value={analytics.publishedArticles} /><Metric label="Tài liệu AI dùng" value={analytics.publishedDocuments} /><Metric label="Tổng chunks" value={analytics.chunkCount} /></div><h3>Nguồn được trích dẫn nhiều nhất</h3>{analytics.topCitedSourceTitles.length ? <ul>{analytics.topCitedSourceTitles.map((source) => <li key={`${source.sourceType}-${source.title}`}>{source.title} <span>· {source.count}</span></li>)}</ul> : <p className="empty">Chưa có trích dẫn AI.</p>}<h3>Chủ đề cần bổ sung</h3>{analytics.gapSuggestions.length ? <ul>{analytics.gapSuggestions.map((gap) => <li key={gap.terms.join('-')}>{gap.terms.join(' ')} <span>· {gap.count}</span></li>)}</ul> : <p className="empty">Chưa có tín hiệu truy vấn thiếu khớp.</p>}</section> }
function OperationsAnalyticsPanel({ analytics }: { analytics: OperationalAnalytics | null }) { if (!analytics) return <section className="kb-admin-card"><p className="empty">Đang tải ticket operations…</p></section>; return <section className="kb-admin-card"><p className="eyebrow">TICKET OPERATIONS</p><h2>SLA & phân luồng</h2><div className="kb-metrics"><Metric label="Đang mở/xử lý" value={analytics.openPending} /><Metric label="Quá SLA" value={analytics.overdueSla} /><Metric label="Đã có phản hồi" value={analytics.firstResponseProxy} /><Metric label="Phản hồi đầu TB (phút)" value={analytics.averageFirstResponseMinutes} /></div><h3>Theo danh mục</h3><div className="operations-breakdown">{analytics.byCategory.map((item) => <span key={item.category}>{labelCategory(item.category as Category)} <b>{item.count}</b></span>)}</div><h3>Theo ưu tiên</h3><div className="operations-breakdown">{analytics.byPriority.map((item) => <span key={item.priority}>{labelPriority(item.priority as Priority)} <b>{item.count}</b></span>)}</div><small>Chỉ số tổng hợp, không hiển thị nội dung hội thoại khách hàng.</small></section> }
...
function MessageBubble({ message, onDownload }: { message: Message; onDownload?: (message: Message) => Promise<void> }) { const internal = message.messageType === 'internal_note'; const label = internal ? 'Ghi chú nội bộ' : message.senderType === 'customer' ? 'Khách hàng' : message.senderType === 'agent' ? 'Nhân viên hỗ trợ' : message.senderType === 'system' ? 'Hệ thống' : 'Support Assistant'; const attachment = message.messageType === 'file'; const citations = message.senderType === 'ai' ? message.metadata?.citations ?? [] : []; return <article className={`bubble ${internal ? 'internal-note-bubble' : message.senderType}`}><small>{label}<time>{new Date(message.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</time></small><p>{message.content}</p>{citations.length > 0 && <div className="message-citations"><span>Nguồn tham khảo:</span>{citations.map((citation) => <em key={`${citation.sourceType}-${citation.title}`}>{citation.title}</em>)}</div>}{attachment && onDownload && <button className="attachment-download" onClick={() => void onDownload(message)}>Tải xuống {message.metadata?.fileName ?? 'tệp đính kèm'}</button>}</article> }
